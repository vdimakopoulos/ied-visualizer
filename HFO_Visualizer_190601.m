...
if(handles.PlotParams.flagSaveScreenshots)
    pushbutton_save_image_Callback(handles.pushbutton_save_image,[],handles)
    for nEvent = 1:handles.nTotalNumberOfEvents
        handles.nEvent = nEvent-1;
        pushbutton_next_event_Callback(handles.pushbutton_next_event,[],handles)
...
if(isfield(InputParams,'YMargin'))
...
% Save screenshots for each event to be validated and close
if(isfield(InputParams,'flagSaveScreenshots')) % Variable plot given
    PlotParams.flagSaveScreenshots = InputParams.flagSaveScreenshots;
else
    PlotParams.flagSaveScreenshots = 0;
end
